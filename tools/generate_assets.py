...
def put(img, name, cx, cy, tile):
    ...
    ATLAS_MAP[name] = (cx * TILE, cy * TILE, tile.size[0], tile.size[1])


def put_px(img, name, x, y, tile):
    img.paste(tile, (x, y), tile)
    ATLAS_MAP[name] = (x, y, tile.size[0], tile.size[1])
...
def write_atlas_header(path, atlas_w=None, atlas_h=None):
    aw = atlas_w if atlas_w is not None else W
    ah = atlas_h if atlas_h is not None else H
    lines = [
        ...
        "#include <string>",
        "namespace tempest {",
        "constexpr int kAtlasW = %d;" % aw,
        "constexpr int kAtlasH = %d;" % ah,
    ...
    for name, rec in sorted(ATLAS_MAP.items()):
        x, y, w, h = rec
...
def build_atlas(include_gale=True):
    global W, H
    W, H = COLS * TILE, ROWS * TILE
    ...
    # Optional legacy 32x32 Gale blobs (slice_gale_sheet.py replaces these).
    if include_gale:
    ...
    write_atlas_header(ROOT / "include" / "Atlas.hpp", img.size[0], img.size[1])
